...
難聴モードの字幕翻訳は、Windows GUIの「難聴翻訳」タブで設定できます。
翻訳用モデル、Timeout、既定の翻訳言語を選択でき、翻訳言語はID・画面表示名・
LLM向け言語名の組み合わせで追加または削除できます。翻訳用モデル欄では既存の
設定済みモデルを選択できるほか、任意のOpenAI互換モデルIDを直接入力できます。
接続先を個別指定していない場合は「LLM」タブの設定を使用します。変更は設定保存後、
SpeechSummarizerサーバーを再起動すると反映されます。
